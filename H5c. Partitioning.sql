...
-----------------------------------------------
-- REMOVE PREVIOUS PARTITIONING FILES IF NEEDED
-----------------------------------------------
...
(
    ...
    FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\sof1.ndf',
    SIZE = 100MB,
    FILEGROWTH = 50MB
)
...
(
    NAME = sof2,
    FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\sof2.ndf',
    SIZE = 100MB,
    FILEGROWTH = 50MB
)
...
(
    NAME = sof3,
    FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\sof3.ndf',
    SIZE = 100MB,
    FILEGROWTH = 50MB
)
...
(
    NAME = sof4,
    FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\sof4.ndf',
    SIZE = 100MB,
    FILEGROWTH = 50MB
)
TO FILEGROUP test4fg;
/***************************
CHECK IN YOUR DATA FOLDER IF THESE FILES HAVE BEEN CREATED WITH THE CORRECT SIZE
***************************/
...
GO
----------------------------------------------------------
-- Create a partitioned table called PostsPartitioned that 
...
select datediff(millisecond, @StartTime, getDate()) as [Exec Time ms: One partition]
...
where score >= 2
group by score;
select datediff(millisecond, @StartTime, getDate()) as [Exec Time ms: Two partitions]

-- Conclusion: selecting from 2 partitions is much slower
